 ...
 */
export const LUNAR_HOUR_TABLE: Record<number, Array<{ branch: string; start: number; end: number }>> = {
  // Tháng 1 âm lịch
  1: [
 ...
 */
export const HOUR_RANGE_DESCRIPTIONS: Record<string, string> = {
 ...
 */
export function normalizeToHanoiTime(
  dateTime: Date,
  birthplace: string,
  isSouthVietnam?: boolean
): TimezoneAdjustment {
 ...
 */
export function getLunarHourBranch(lunarMonth: number, solarHours: number, solarMinutes: number): string {
  const table = LUNAR_HOUR_TABLE[lunarMonth];
  if (!table) {
 ...
 */
export function getHourStem(dayStemIndex: number, hourBranchIndex: number): string {
 ...
 */
export function checkTransitionWarning(
  lunarMonth: number,
  solarHours: number,
  solarMinutes: number
): string | null {
 ...
 */
export function formatHourRange(lunarMonth: number, branch: string): string {
  const table = LUNAR_HOUR_TABLE[lunarMonth];
  if (!table) return '';
  ...
  const formatTime = (minutes: number): string => {
    if (minutes === 1440 || minutes === 0) return '24h00';
 ...
 */
export function calculateLunarHour(
  lunarMonth: number,
  solarHours: number,
  solarMinutes: number,
  dayStemIndex?: number
